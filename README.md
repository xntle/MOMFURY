...
- Health: 2000 HP
- **Attack 1**: Shoots diapers in all 8 directions simultaneously
- **Attack 2**: Summons 5 minions in a circle formation
- **Pattern**: Alternates between abilities randomly
- **Strategy**:
  - For diapers: Move diagonally between the 8 directions
  - For summons: Immediately clear the minions with Broom or focus fire with Rice Gun
  - Most dangerous boss due to summoning - don't let minions stack!
...
**Your high scores are saved in your browser**, so compete against yourself and improve with each run!
...
**Player Movement System** - Implemented 8-directional movement with decoupled movement and animation systems for intuitive twin-stick shooter controls. Movement uses WASD/Arrow key input to build a direction vector that is normalized (lines 142-158) to prevent diagonal movement from being √2 faster than cardinal directions. The key innovation is separating movement direction from visual facing: while the player moves using input direction, the character animations face the mouse cursor position (lines 321-326). This creates more intuitive aiming - players naturally expect their character to face where they're shooting, not where they're walking. Implemented by calculating `mouse_dir = (mouse_pos - global_position).normalized()` and passing it to `_get_anim_name()` instead of the movement vector. This design choice significantly improves game feel in a top-down shooter where you need to strafe while aiming, demonstrating separation of concerns in game systems and prioritizing player intuition over strict simulation. Movement speed applies `slow_multiplier` for beer poison debuff integration and uses `move_and_slide()` for collision-aware physics movement. [Player movement input](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L142-L158) | [Mouse-based animation](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L321-L326)

**Dodge Roll Mechanics** - Created invincibility frame system using Godot's collision layer manipulation for temporary invulnerability during rolls. When spacebar is pressed (lines 178-198), the player enters roll state with `is_rolling = true` and collision layer switches from 1 (normal) to 8 (intangible). This layer change makes the player undetectable by enemies and projectiles (which only mask layer 1), creating true invincibility frames. Roll direction is intelligently determined: if player is currently inputting movement, roll in that direction; otherwise, roll in `last_move_dir` (the last direction faced), preventing awkward stationary rolls. Uses `move_and_collide()` instead of `move_and_slide()` during roll for precise 8x speed boost control (400 units/sec vs 50 normal). Implements cooldown system (`cooldown_timer`) to prevent spam, ensuring tactical usage. After roll completes (`roll_timer <= 0`), collision layer reverts to 1 (line 138). This creates high-skill gameplay where players can dodge through bullet hell patterns and enemy swarms by timing rolls precisely, rewarding mastery similar to games like Enter the Gungeon. The intangibility timer system (lines 227-231) includes guardrails to handle multiple sources of invincibility without conflicts, checking `if not is_rolling` before reverting layer. [Roll input detection](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L178-L198) | [Collision layer switching](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L138) | [Intangibility management](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L227-L231)

**Weapon Switching System** - Designed object-oriented weapon system using enum-based state management and polymorphic weapon interface following OOP inheritance principles. Defined `enum Weapon { SLIPPER, RICE_MACHINE, BROOM }` (line 11) as type-safe weapon identifiers, preventing invalid weapon states. Each weapon inherits from `WeaponBase` class, ensuring consistent interface with `attack()` method. Implemented polymorphic weapon retrieval via `_get_current_weapon()` (lines 437-445) which returns `WeaponBase` reference using match statement, enabling uniform weapon interaction regardless of specific type. Weapon switching uses cyclic state machine pattern in `_switch_weapon()` (lines 448-463): Slipper → Rice → Broom → Slipper, implemented with match/case exhaustive pattern matching. Visibility management system (lines 466-474) ensures only `current_weapon` node is visible by setting `visible` property on weapon Node2D children, maintaining single active weapon display. Weapons are attached as child nodes (`$slip`, `$Ricechine`, `$Broom` at lines 42-44) following component-based architecture - each weapon is self-contained scene with own attack logic, cooldowns, and projectile spawning. The `_shoot_current_weapon()` method (lines 431-434) demonstrates polymorphism: calls `weapon.attack()` without knowing concrete weapon type. Added weapon layering system (lines 477-489) that adjusts z-index based on mouse direction - weapons appear behind player when aiming up, in front when aiming down, creating proper depth perception. This OOP structure makes adding new weapons trivial: create WeaponBase subclass, add to enum, update match statements. Demonstrates inheritance, polymorphism, encapsulation, and state pattern from course OOP principles. [Weapon enum & polymorphism](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L11-L12) | [Weapon switching FSM](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L448-L463) | [Polymorphic attack](https://github.com/xntle/MOMFURY/blob/main/scripts/player.gd#L431-L445)

**Enemy AI Behaviors** - Developed multiple enemy types with distinct state machine-based AI using Godot's NavigationAgent2D for pathfinding. Cockroach AI implements three-state behavior pattern: normal movement, warning pause, and dash attack. Uses randomized dash timer (`dash_timer = randf_range(1.0, 3.0)` at line 24) to create unpredictable attack timing. Critical telegraphing mechanic: when timer reaches 0.5s threshold (line 74), cockroach freezes in place for 0.5 seconds, visually warning players before executing 250 unit/sec dash attack (line 56). This creates skill-based counterplay where observant players can dodge roll through the dash. After dash completes, enters bounce-back state (`velocity = -4.0 * direct_dir * speed` at line 62) that pushes cockroach away from player, preventing stunlock and creating spacing. AI integrates NavigationAgent2D pathfinding (`agent.get_next_path_position()` at lines 50-52) to navigate around walls during pursuit, avoiding naive direct-line movement that gets stuck on obstacles. Enemy direction constantly updates with `rotation = direct_dir.angle() + deg_to_rad(90)` (line 77) to face player, improving visual clarity. All enemies use collision layers 2 (enemy layer) and mask walls/transparent walls for environment navigation. Implemented health/damage systems with signal-based communication: enemies call `player.take_damage()` on collision and receive `take_damage()` calls from projectiles. This demonstrates AI state machines, pathfinding integration, and signal-based decoupled architecture from course AI and design pattern lectures. [Cockroach dash AI](https://github.com/xntle/MOMFURY/blob/main/scripts/Enemies/cockroach.gd#L40-L75) | [Navigation pathfinding](https://github.com/xntle/MOMFURY/blob/main/scripts/Enemies/cockroach.gd#L50-L71) | [Warning telegraph](https://github.com/xntle/MOMFURY/blob/main/scripts/Enemies/cockroach.gd#L73-L75)

**Health Drop System** - Created procedural health pickup spawning with percentage-based drop rates using probabilistic RNG system. Implemented in `_drop_health()` method (lines 106-111) called on enemy death, uses `randf() < drop_chance` comparison where `drop_chance` is exported float variable unique to each enemy type (cockroach: 0.2, poop: 0.3, A-minus: 0.5). Higher-threat enemies have higher drop rates, creating risk-reward incentive to prioritize dangerous targets. Health pickups instantiated as scenes (`health_pickup_scene.instantiate()`) and spawned at enemy death position, with 3-second lifetime creating time pressure - players must collect before despawn. This forces tactical decision-making: pursue aggressive plays for health drops vs safe positioning. Drop system integrated with enemy `take_damage()` death detection: when `health <= 0`, calls `_drop_health()` before `queue_free()`. Uses Godot's scene instantiation pattern for runtime object creation, demonstrating factory pattern and procedural content generation. The percentage-based randomization ensures varied gameplay - sometimes lucky drops, sometimes droughts - maintaining unpredictability in endless mode. [Drop system implementation](https://github.com/xntle/MOMFURY/blob/main/scripts/Enemies/Aminus.gd#L106-L111) | [Drop rates per enemy](https://github.com/xntle/MOMFURY/blob/main/scripts/Enemies/Aminus.gd#L13)
